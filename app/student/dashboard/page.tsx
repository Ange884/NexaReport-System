...
import { Clock, AlertCircle, CheckCircle2, FileText, ArrowUpRight, Loader2, RefreshCw } from "lucide-react";
import Link from "next/link";
import { fetchMyIssues } from "@/app/lib/api";
import type { IssueDashboardResponse, IssueResponseDto, IssueStatus } from "@/app/lib/types";
...
function StatusBadge({ status }: { status: IssueStatus }) {
  ...
  return (
    <span className={`rounded-lg px-2.5 py-1 text-[11px] font-black uppercase tracking-wider ${cls}`}>
      {label}
    </span>
  );
}
...
export default function StudentDashboard() {
  ...
  // Most recent 5 issues for the table
    ...
    <div className="relative animate-fade-in">
      ...
      {error && !isLoading && (
        <div className="mb-6 flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-5 py-4 text-sm font-bold text-red-600">
          ...
          <button
            onClick={load}
            className="flex items-center gap-1.5 rounded-lg bg-red-100 px-3 py-1.5 text-xs font-black text-red-600 transition hover:bg-red-200"
          >
                ...
                {recentIssues.map((issue) => (
                  <tr key={issue.id} className="group transition-colors hover:bg-[var(--background)]">
                    <td className="py-4 pr-4">
                      <Link href={`/student/dashboard/issues/${issue.id}`}>
                        <span className="text-sm font-bold text-[var(--foreground)] transition group-hover:text-[var(--accent)] cursor-pointer line-clamp-1">
                          {issue.title}
                        </span>
                      </Link>
                    </td>
                    <td className="py-4 pr-4 text-xs font-bold text-[var(--muted)] capitalize">
                      {issue.category}
                    </td>
                    <td className={`py-4 pr-4 text-xs font-black ${priorityColors[issue.priority] ?? ""}`}>
                      {issue.priority}
                    </td>
                    <td className="py-4 pr-4">
                      <StatusBadge status={issue.status} />
                    </td>
                    <td className="py-4 text-right text-xs font-bold text-[var(--muted)]">
