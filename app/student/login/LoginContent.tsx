...
import { getDefaultRoute, isAdminRole } from "@/app/lib/types";

export default function StudentLoginContent() {
  ...
  // If already authenticated, redirect to their default route
  useEffect(() => {
    if (isAuthenticated()) {
  ...
  async function handleLogin(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Otherwise route to their natural default, honouring any ?next= redirect
        ...
        <div className="relative w-full max-w-[480px] overflow-hidden rounded-lg border border-[rgba(33,19,13,0.08)] bg-white/95 px-14 py-14 shadow-[0_24px_60px_rgba(33,19,13,0.12)] backdrop-blur-xl">
          ...
          <form className="mt-6 space-y-5" onSubmit={handleLogin}>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold uppercase tracking-wider text-[#21130D]">Email Address</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold uppercase tracking-wider text-[#21130D]">Password</label>
              <div className="relative flex items-center">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm font-medium text-[#718096]">
                <input type="checkbox" className="h-3.5 w-3.5 rounded border-[#e2e8f0] accent-[#21130D]" />
                Remember me
              </label>
