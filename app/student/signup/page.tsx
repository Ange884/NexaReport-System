"use client";

import React, { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { Mail, Lock, Eye, EyeOff, User, UserPlus } from "lucide-react";

export default function StudentSignUp() {
  const [showPassword, setShowPassword] = useState(false);
  const router = useRouter();

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push("/student/dashboard");
  };

  return (
    <main className="relative min-h-screen overflow-hidden bg-white flex items-center justify-center">
      {/* Animated background grid */}
      <div
        className="pointer-events-none absolute inset-0 z-0"
        style={{
          backgroundSize: "40px 40px",
          backgroundImage:
            "linear-gradient(to right,rgba(33,19,13,0.04) 1px,transparent 1px),linear-gradient(to bottom,rgba(33,19,13,0.04) 1px,transparent 1px)",
        }}
      />
      {/* Decorative blobs */}
      <div className="pointer-events-none absolute -left-[10%] -top-[20%] h-[600px] w-[600px] animate-pulse rounded-full bg-[radial-gradient(circle,rgba(33,19,13,0.06)_0%,rgba(255,255,255,0)_70%)] blur-[40px]" />
      <div className="pointer-events-none absolute -bottom-[20%] -right-[10%] h-[600px] w-[600px] animate-pulse rounded-full bg-[radial-gradient(circle,rgba(33,19,13,0.06)_0%,rgba(255,255,255,0)_70%)] blur-[40px]" />

      {/* Content */}
      <div className="relative z-10 flex w-full max-w-5xl items-center justify-between gap-16 px-8 py-10 md:px-12">
        {/* Left showcase */}
        <div className="hidden flex-1 flex-col justify-center md:flex">
          <h1
            className="mb-6 text-[5rem] leading-[1.1] tracking-wide text-[#21130D]"
            style={{ fontFamily: "'Creepster', cursive", textShadow: "2px 2px 4px rgba(33,19,13,0.1)" }}
          >
            Speak Up!
          </h1>
          <p className="max-w-[80%] text-xl font-medium leading-relaxed text-[#21130D] opacity-80">
            A fast, secure, and hassle-free platform for students to report
            issues easily to the staff. Your voice matters here.
          </p>
        </div>

        {/* Auth card */}
        <div className="relative w-full max-w-[480px] overflow-hidden rounded-lg border border-[rgba(33,19,13,0.08)] bg-white/95 px-14 py-14 shadow-[0_24px_60px_rgba(33,19,13,0.12)] backdrop-blur-xl">
          {/* Top accent bar */}
          <div className="absolute left-0 top-0 h-1.5 w-full bg-[#21130D]" />

          {/* Header */}
          <div className="mb-10 text-center">
            <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-[rgba(33,19,13,0.05)] text-[#21130D]">
              <UserPlus size={32} strokeWidth={2.5} />
            </div>
            <h2 className="text-[2.2rem] font-extrabold tracking-tight text-[#21130D]">
              Create Account
            </h2>
            <p className="mt-1 text-base font-medium text-[#718096]">
              Join the student portal to get started
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Full Name */}
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-[#21130D]">
                Full Name
              </label>
              <div className="relative flex items-center">
                <User
                  className="pointer-events-none absolute left-5 text-[#718096]"
                  size={20}
                />
                <input
                  type="text"
                  placeholder="John Doe"
                  required
                  className="w-full rounded-[14px] border-2 border-[#efefef] bg-white py-[1.1rem] pl-[3.5rem] pr-5 text-base font-medium text-[#21130D] transition-all placeholder:font-normal placeholder:text-[#adb5bd] focus:border-[#21130D] focus:shadow-[0_0_0_5px_rgba(33,19,13,0.05)] focus:outline-none"
                />
              </div>
            </div>

            {/* Email */}
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-[#21130D]">
                Email Address
              </label>
              <div className="relative flex items-center">
                <Mail
                  className="pointer-events-none absolute left-5 text-[#718096]"
                  size={20}
                />
                <input
                  type="email"
                  placeholder="student@university.edu"
                  autoComplete="email"
                  required
                  className="w-full rounded-[14px] border-2 border-[#efefef] bg-white py-[1.1rem] pl-[3.5rem] pr-5 text-base font-medium text-[#21130D] transition-all placeholder:font-normal placeholder:text-[#adb5bd] focus:border-[#21130D] focus:shadow-[0_0_0_5px_rgba(33,19,13,0.05)] focus:outline-none"
                />
              </div>
            </div>

            {/* Password */}
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-[#21130D]">
                Password
              </label>
              <div className="relative flex items-center">
                <Lock
                  className="pointer-events-none absolute left-5 text-[#718096]"
                  size={20}
                />
                <input
                  type={showPassword ? "text" : "password"}
                  placeholder="Create a strong password"
                  required
                  className="w-full rounded-[14px] border-2 border-[#efefef] bg-white py-[1.1rem] pl-[3.5rem] pr-14 text-base font-medium text-[#21130D] transition-all placeholder:font-normal placeholder:text-[#adb5bd] focus:border-[#21130D] focus:shadow-[0_0_0_5px_rgba(33,19,13,0.05)] focus:outline-none"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  aria-label="Toggle password visibility"
                  className="absolute right-4 flex items-center justify-center rounded-lg p-1 text-[#718096] transition hover:bg-[rgba(33,19,13,0.05)] hover:text-[#21130D]"
                >
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
            </div>

            <button
              type="submit"
              className="mt-2 flex w-full items-center justify-center gap-3 rounded-[14px] bg-[#21130D] py-5 text-[1.1rem] font-bold text-white shadow-[0_12px_32px_rgba(33,19,13,0.08)] transition-all hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(33,19,13,0.12)] active:scale-[0.97]"
            >
              Register Now
            </button>
          </form>

          <div className="mt-8 text-center text-base font-medium text-[#718096]">
            Already have an account?{" "}
            <Link
              href="/student/login"
              className="ml-1 font-extrabold text-[#21130D] hover:opacity-80"
            >
              Sign In
            </Link>
          </div>
        </div>
      </div>

      {/* Creepster font */}
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Creepster&display=swap');`}</style>
    </main>
  );
}
